"""Spreadsheet import tests for term column and legacy header schemas."""
...
class TestTermSpreadsheetImport(unittest.TestCase):
    def test_normalize_sheet_headers_accepts_v3_legacy_without_term(self) -> None:
    ...
    def test_import_v3_legacy_defaults_term_to_full(self) -> None:
        ...
        self.assertEqual(section["term"], "full")
        self.assertIsNone(section.get("assigned_half"))

    def test_import_reads_term_and_assigned_half(self) -> None:
        ...
        payload = parse_scheduling_input_from_excel_bytes(_minimal_workbook_bytes(headers, row))
        section = payload["sections"][0]
        self.assertEqual(section["term"], "half_any")
