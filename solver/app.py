...
def _section_to_dict(section) -> dict:
    ...
    return section.to_dict() if hasattr(section, "to_dict") else {
        ...
        "assigned_half": getattr(section, "assigned_half", None),
    }
...
def _build_half_choice_vars(
    ...
    for section in input_data.sections:
        ...
        if resolve_section_term(section_dict) != TERM_HALF_ANY:
            continue
...
def _add_slot_term_partition_constraints(
    ...
    for idx, (used, term, half_choice) in enumerate(group_usages):
        if term == "full":
            ...
            g_h2.append(used)
        elif term == TERM_HALF_ANY and half_choice is not None:
            g_h1.append(
...
def _solve_schedule(input_data: SchedulingInput):
    ...
    for section_id, options in options_by_section.items():
        ...
        if resolve_section_term(section_dict) == TERM_HALF_ANY and section_id in half_choice_vars:
